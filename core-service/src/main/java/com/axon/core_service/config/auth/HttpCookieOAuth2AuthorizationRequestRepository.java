...
public class HttpCookieOAuth2AuthorizationRequestRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    public OAuth2AuthorizationRequest loadAuthorizationRequest(HttpServletRequest request) {
        log.info("Attempting to load authorization request from cookie");
        return CookieUtils.getCookie(request, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME)
                .map(cookie -> {
                    log.info("OAuth2 authorization request cookie found.");
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest, HttpServletRequest request, HttpServletResponse response) {
        if (authorizationRequest == null) {
            log.info("AuthorizationRequest is null, removing cookies.");
        ...
        if (StringUtils.isNotBlank(redirectUriAfterLogin)) {
            log.info("Saving redirect URI to cookie: {}", redirectUriAfterLogin);
